...
export const calculateMacros = (data) => {
    ...
    data.map((value) => {
        macros.kcal = parseInt(value.ingredient.kcal) / 100 * parseInt(value.ingredient_weight) + macros.kcal;
        macros.protein = parseInt(value.ingredient.protein) / 100 * parseInt(value.ingredient_weight) + macros.protein;
    ...
    macros.macroPercentages.protein = macros.kcalPerMacro.protein / macros.kcalPerMacro.total * 100;
    macros.macroPercentages.carbs = macros.kcalPerMacro.carbs / macros.kcalPerMacro.total * 100;
    macros.macroPercentages.fat = macros.kcalPerMacro.fat / macros.kcalPerMacro.total * 100;
    macros.macroPercentages.total = macros.macroPercentages.fat + macros.macroPercentages.carbs + macros.macroPercentages.protein
